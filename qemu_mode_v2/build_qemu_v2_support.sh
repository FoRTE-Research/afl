...
# will be written to ../afl-qemuv2-trace.
#
...
cp -f "${CPU_TARGET}-linux-user/qemu-${CPU_TARGET}" "../../afl-qemuv2-trace" || exit 1

cd ..
ls -l ../afl-qemuv2-trace || exit 1

echo "[+] Successfully created '../afl-qemuv2-trace'."
...
if [ "$ORIG_CPU_TARGET" = "" ]; then
  ...
  echo 0 | ./afl-showmap -m none -QQ -q -o .test-instr0 ./test-instr || exit 1
  echo 1 | ./afl-showmap -m none -QQ -q -o .test-instr1 ./test-instr || exit 1
  ...
  if [ "$DR" = "0" ]; then
    ...
    echo "[-] Error: afl-qemuv2-trace instrumentation doesn't seem to work!"
    exit 1
